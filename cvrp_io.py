    ...
    * ewt is the EDGE_WEIGHT_TYPE
    ...
    with open(file_name, "r") as f:
            ...
            # Section handling
                ...
                if "EDGE_WEIGHT_SECTION" in line:
                    ...
                    if edge_weight_format == "LOWER_ROW":
                        ij_section_pos["j"] = 1
                    elif edge_weight_format == "UPPER_ROW" or edge_weight_format == "LOWER_COL":
                        ij_section_pos["i"] = 1
                    ...
                    if section == "EDGE_WEIGHT_SECTION":
                        ...
                        for d in distances:
                            ...
                            if edge_weight_format == "LOWER_ROW":
                                ...
                                or edge_weight_format == "LOWER_COL"
                            ):
                                # incrementer
                                ij_section_pos["i"] += 1
                        ...
                        try:
                            ...
                            # Assume that this section specifies the
                            # coordinates of the depot and the depot is not
                            # contained in the nodes yet.
                            depot_coords = [float(x) for x in line.split(" ")]
                            points.insert(0, depot_coords)
                            points = points[:-1]
        ...
        if edge_weight_type == "EXPLICIT" and not (
            (
                (edge_weight_format in ["FULL_MATRIX", "LOWER_ROW", "LOWER_DIAG_ROW"])
                and ij_section_pos["i"] == 0
                and ij_section_pos["j"] == len(D)
            )
            or (
                edge_weight_format in ["UPPER_ROW", "LOWER_COL"]
                and ij_section_pos["i"] == len(D)
                and ij_section_pos["j"] == len(D) - 1
            )
            or (
    ...
    return ProblemDefinition(N, points, dd_points, demands, D, C, edge_weight_type, vehicles)
...
def write_TSPLIB_file(
    tsplib_file_path, D, d=None, C=None, L=None, selected_idxs=None, float_to_int_precision=None
):
